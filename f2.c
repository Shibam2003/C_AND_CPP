...
int main(){
	...
	fp2 = fopen("f2.txt","w");//opening the abc file in write mode
	if(fp2==NULL){
		printf("There is error in opening the file");
		exit(0);
	}
	...
	fp3 = fopen("f3.txt","w");//opening the abc file in write mode
	if(fp3==NULL){
		printf("There is error in opening the file");
		exit(0);
	}
	
	while(!feof(fp2))//checking whether the end of file marker has been reached or not
	{
		ch = getc(fp2);//the getc method is used to extract a character from the file
		putc(ch,fp3);//the putc method is used to write the character to a file 
